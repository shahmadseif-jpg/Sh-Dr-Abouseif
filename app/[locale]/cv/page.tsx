...
export async function generateMetadata({
  ...
  const ar = locale === 'ar';
  return {
    title: `${ar ? 'السيرة العلميّة' : locale === 'es' ? 'CV académico' : 'Academic CV'} — ${ar ? 'د. أحمد أبو سيف' : 'Dr. Ahmed Abouseif'}`,
    description: ar
...
const bio: Record<Loc, string> = {
  ...
  en: 'Dr. Ahmed Abouseif holds a doctorate (al-ʿĀlamiyya) in Tafsīr and Qurʾanic Sciences from al-Azhar University and serves as President of the American Imams Academy (AIA). He has nearly thirty-five years of daʿwah, teaching, and preaching since 1991, with more than 2,600 lectures and lessons delivered in three languages, alongside peer-reviewed research in tafsīr, fiqh, and the jurisprudence of Muslim minorities.',
  es: 'El Dr. Ahmed Abouseif posee un doctorado (al-ʿĀlamiyya) en Tafsir y Ciencias Coránicas por la Universidad de Al-Azhar y es presidente de la Academia Americana de Imames (AIA). Cuenta con casi treinta y cinco años de predicación, enseñanza y oratoria desde 1991, con más de 2.600 conferencias y lecciones impartidas en tres idiomas, además de investigaciones arbitradas en tafsir, jurisprudencia y la jurisprudencia de las minorías musulmanas.',
};

const articleCategoryLabels: Record<ArticleCategory, Record<Loc, string>> = {
  'maqasid-tafsir': { ar: 'التفسير المقاصدي', en: 'Maqāṣid-Based Tafsīr', es: 'Tafsir basado en los objetivos' },
  'wisdom-insights': { ar: 'حِكَمٌ وبصائر', en: 'Wisdom & Insights', es: 'Sabiduría y perspicacias' },
  'prophetic-light': { ar: 'قبسٌ من نور النبوّة', en: 'Glimmers of Prophetic Light', es: 'Destellos de la Luz Profética' },
  imamship: { ar: 'الإمامة والقيادة', en: 'Imamship & Leadership', es: 'Imamato y liderazgo' },
  civilization: { ar: 'الإسلام والحضارة', en: 'Islam & Civilization', es: 'Islam y civilización' },
  family: { ar: 'الأسرة', en: 'Family', es: 'Familia' },
  fiqh: { ar: 'الفقه', en: 'Fiqh', es: 'Jurisprudencia' },
};
...
export default async function CvPage({
  ...
  const orderedCats: ArticleCategory[] = [
    'maqasid-tafsir', 'wisdom-insights', 'prophetic-light', 'imamship', 'civilization', 'family', 'fiqh',
  ];
